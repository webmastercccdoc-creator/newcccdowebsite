...
export default function CollegeEducation() {
    ...
    useEffect(() => {
        ...
        fetch('/api/news?department=coe')
            .then((response) => {
                if (!response.ok) throw new Error('Failed to fetch COE news');
                return response.json();
            })
            .then((data) => {
                if (isMounted) {
                    setCoeNews(Array.isArray(data) ? data : []);
                }
            })
            .catch(() => {
                if (isMounted) setCoeNews([]);
            })
            .finally(() => {
    ...
    // --- News Items ---
    const newsItems = Array.isArray(coeNews) && coeNews.length > 0 ? coeNews.slice(0, 6).map((item, index) => ({
        id: item.id || `coe-article-${index}`,
        date: formatDate(item.date),
        title: item.title || 'COE News',
        excerpt: stripHtml(item.content || 'The latest update from the College of Education.'),
        category: item.category || 'Education',
        department: item.department || 'COE',
        image: normalizeImagePath(item.image_path || item.image || item.image_url),
        alt: item.title || 'COE News',
        link: `/news/${item.id}`,
        ...
        { id: 'coe-1', date: 'Coming Soon', title: 'College of Education News', excerpt: 'Updates and articles from the College of Education will be displayed here once available.', category: 'Announcement', image: 'https://placehold.co/600x400/1a237e/ffffff?text=COE+News', link: '#', sdgNumbers: [4] },
        { id: 'coe-2', date: 'Coming Soon', title: 'Teacher Training Workshop', excerpt: 'Faculty and students participate in the latest pedagogical advancements workshop.', category: 'Event', image: 'https://placehold.co/600x400/0d47a1/ffffff?text=Workshop', link: '#', sdgNumbers: [4] },
        { id: 'coe-3', date: 'Coming Soon', title: 'New Curriculum Launch', excerpt: 'The College of Education proudly announces the rollout of its enhanced curriculum.', category: 'Academic', image: 'https://placehold.co/600x400/1565c0/ffffff?text=Curriculum', link: '#', sdgNumbers: [4] }
    ];
    ...
    const coreValues = [
        ...
        { title: "Innovation", desc: "Proactive seeking of creative solutions and embracing modern pedagogical breakthroughs.", icon: "M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456z" },
        { title: "Adaptability", desc: "Remaining resilient and flexible in responding to the evolving educational landscape.", icon: "M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" },
        { title: "Lifelong Learning", desc: "Continuous pursuit of professional development and advanced scholarship in education.", icon: "M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" }
    ];
    ...
    return (
        ...
        >
            <style>{`
                ...
                #drag-container .coe-3d-card img {
                  width: 100%;
                  height: 100%;
                  object-fit: cover;
                ...
                .coe-3d-date { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #34d399; font-weight: 700; display: block; margin-bottom: 8px; }
                .coe-3d-title {
                  font-size: 18px;
                  font-weight: 700;
                  margin: 0 0 8px;
                ...
                .coe-3d-sdg-container { display: flex; flex-wrap: wrap; gap: 6px; pointer-events: none; }
                .coe-3d-sdg { background: rgba(52, 211, 153, 0.15); color: #34d399; border: 1px solid rgba(52, 211, 153, 0.3); padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; text-transform: uppercase; }
                    ...
                    <div className="grid md:grid-cols-12 gap-8 md:gap-12 items-start">
                        ...
                        >
                            ...
                            <div className="text-center mt-8 w-full max-w-xs mx-auto">
                                ...
                                <p className="mt-1 text-sm text-slate-400 font-medium">DevEdD, PhD, PACUCOA Accreditor</p>
                        ...
                        <div className="md:col-span-7 md:pt-4">
                            ...
                            >
                                {[
                                    "Doctor in Development Education (DevEdD)",
                                        ...
                                        <div id="spin-container" ref={spinRef} style={{ margin: '0 auto', width: '220px', height: '320px', position: 'relative', transformStyle: 'preserve-3d', top: '8px' }}>
                                            {newsItems.map((item, index) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="coe-3d-overlay">
                                                        <span className="coe-3d-date">{item.date} • {item.category}</span>
                                                        <h3 className="coe-3d-title">{item.title}</h3>
                                                        <p className="coe-3d-excerpt">
                                                            {item.excerpt.length > 90 ? item.excerpt.substring(0, 90) + '...' : item.excerpt}
                                                        </p>
                                                        {item.sdgNumbers && item.sdgNumbers.length > 0 && (
                                                            <div className="coe-3d-sdg-container">
                                                                {item.sdgNumbers.map(num => (
                                                                    <span key={num} className="coe-3d-sdg">SDG {num}</span>
                                                                ))}
                                                            </div>
                                                        )}
                                                    </div>
