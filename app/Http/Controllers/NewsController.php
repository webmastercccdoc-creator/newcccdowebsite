...
class NewsController extends Controller
{
    ...
    {
        $query = DB::table('news_articles as na')
            ->select([
                'na.id',
                'na.title',
                'na.content',
        ...
        if ($departmentFilter !== '') {
            ...
            $query->where(function ($q) use ($normalized, $patterns) {
                $q->whereRaw('LOWER(na.department) = ?', [$normalized])
                    ->orWhereRaw('LOWER(REPLACE(REPLACE(REPLACE(REPLACE(na.department, "-", ""), " ", ""), "&", ""), "/", "")) LIKE ?', ['%' . $normalized . '%']);

                foreach ($patterns as $pattern) {
                    $q->orWhereRaw('LOWER(REPLACE(REPLACE(REPLACE(REPLACE(na.department, "-", ""), " ", ""), "&", ""), "/", "")) LIKE ?', ['%' . $pattern . '%']);
                }
            });
        }
